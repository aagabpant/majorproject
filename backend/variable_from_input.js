...
export default function executeVariableFromInput(requestParams) {
  const {
    quarters,
    banks,
    filepath,
  ...
  const functionname = "variable_extraction_from_newfile";
  ...
  if (pythonProcess.error) {
    ...
    try {
      ...
      const resultFilePath = `C:/Users/Aagab/PycharmProjects/pythonProject1/${access_token}/z output/variable_from_input.json`;
      ...
      const modifiedResult = Array.isArray(resultContent)
        ? {
            quarters: resultContent.map((entry) => entry.quarter),
