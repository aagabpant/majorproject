...
export default function executeBankandVariableExisting(requestParams) {
  const {
    quarters,
    banks,
    filepath,
  ...
  const functionname = "bank_and_variable_extraction_from_oldfiles";
  ...
  if (pythonProcess.error) {
    ...
    try {
      ...
      const resultFilePath = `C:/Users/Aagab/PycharmProjects/pythonProject1/${access_token}/z output/bank_and_variable_existing.json`;
      ...
      const modifiedResult = Array.isArray(resultContent)
        ? {
            quarter: resultContent.map((entry) => entry.quarter),
